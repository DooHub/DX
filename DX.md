...
## 1) 정형 Data - 높은 예측성능 Random Forest사용 ex)은행 고객의 연체 예측
## 2) 이상동작 감지 - 이상치 값(Data)을 취득하기 어려움, Data의 unblance 함. Deep Learning 기반의 지도 학습 방법 이용 ex) 부정맥 환자 예측
사람이 판단하기 전에 AI를 이용하여 사전 감지, 감지 된 결과를 사람이 다시 확인
## 3) 실시간 측정 Data처리  - 공정 모니터링으로 공정 문제 예측 및 공정 효율 향상을 위해 어느 변수를 조절해야 하는 예측 (Deep Learning 활용)
## 4) 이미지 처리 -정상과 비정상 이미지 처리 : 비지도 학습의 이상치 탐지 알고리즘 이용(Anomaly Detection) 이용 with Computer Vision 알고리 활용
비정상 이미지 추출 --> 사람이 이상 여부 판단 --> 비정상 이미지에 대한 이상 유무 update 순환 구조로 알고리즘 최적화  
### 이상치 탐지 알고리즘을 사용하는 이유 : 
. 정상,비정상 데이트의 비율이 압도적으로 불균형함  
. 인력 및 시간 부족으로 인한 데이터의 정상여부를 정리하는 과정을 진행할 수 없는 무제
